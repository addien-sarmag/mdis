...
endforeach;	
	...
	$col[] = array('text' => $res['fakturPajakCustomerListHargaValas'] , 'width' => $w[$n++], 'align' => 'R');
	...
	$pdf->Cell($w[2], 5,$res['fakturPajakCustomerTotal'],1,0,'R');
	$pdf->Cell($w[3], 5,'',1,1,'R');
	
	$pdf->Cell($w[0] + $w[1] , 5,'Dikurangi Potongan Harga ',1,0);
	$pdf->Cell($w[2], 5,$res['fakturPajakCustomerDiskon'].' %',1,0,'R');
	$pdf->Cell($w[3], 5,'',1,1,'R');
	
	$pdf->Cell($w[0] + $w[1] , 5,'Dikurangi Uang Muka Yang Telah Diterima',1,0);
	$pdf->Cell($w[2], 5,'',1,0,'R');
	$pdf->Cell($w[3], 5,'',1,1,'R');
	
	$pdf->Cell($w[0] + $w[1] , 5,'Dasar Pengenaan Pajak',1,0);
	$pdf->Cell($w[2], 5,$res['fakturPajakCustomerDasarPengenaanPajak'],1,0,'R');
	$pdf->Cell($w[3], 5,'',1,1,'R');
	
	$pdf->Cell($w[0] + $w[1] , 5,'PPN = 10 % x Dasar Pengenaan Pajak ',1,0);
	$pdf->Cell($w[2] , 5,$res['fakturPajakCustomerPpn'],1,0,'R');
	$pdf->Cell($w[3], 5,'',1,1,'R');
