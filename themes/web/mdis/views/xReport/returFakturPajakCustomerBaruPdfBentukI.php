...
//	$filename = 'Credit-Note '.$noCN.'.pdf';
	...
	$pdf->Cell(70,8,'Tanggal : '. ina_date($rfpTanggal)/*. ina_date($cnTanggal)*/,'RT',1,'L');
...
//	$thead2  = array( '  Urut   ','                                                                   ','                 ','         '.$matauang.'     ','                '.$matauang.'  ');
	...
	$totalRupiahSeluruhnya = $totalRupiahSeluruhnya + $totalRupiah;
	$columns[] = $col;
	}else{
		$col[] = array('text' => $urut++ . '.', 'width' => $w[$n++], 'align' => 'C','linewidth'=>0.2);
		$col[] = array('text' => $res['barangNama']." ".$res['barangKode']. "        ".$res['returFakturPajakCustomerListJumlah']." ".$res['satuanSingkatan'], 'width' => $w[$n++], 'align' => 'L');
...
//	$hrgPajakMin = 213000;
	...
	$col = array();
	$col[] = array('text' =>  'PPN yang diminta kembali' , 'width' => $w[0] + $w[1]  , 'align' => 'L','linewidth'=>0.2);
...
//	$hrgPajakMin = 213000;
	...
	$pdf->Cell(0,5,'','LR',1);
	$pdf->Cell(120,5,'','L');
	$pdf->Cell(0,5,' Jakarta, 13 Oktober 2010'/*. ina_date( $cnTanggal )*/,'R',1,'C');
	$pdf->Cell(120,5,'','L');
