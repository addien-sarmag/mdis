...
class CI_Loader extends MB_Loader {
	...
	{
		$CI = get_instance();
		if ($CI->config->item('web_type'))
			$this->_web_type = $CI->config->item('web_type');
		if ($CI->config->item('themes'))
			$this->_themes = $CI->config->item('themes');
		$path = FCPATH  . 'themes' . DS . $this->_web_type . DS . $this->_themes . DS  ;
		$mb_file = $this->_mb_load('views/'.$view, $module , $path);
	...
	function _ci_autoloader()
	{
		...
		/// edit by: febru line 700 => "@" + script original
